...
function EditableNote({ note, disabled, onRemove, onSave }: { note: Note; disabled: boolean; onRemove: NotesProps["onRemove"]; onSave: NotesProps["onSave"] }) {
  ...
  }, [content, disabled, expiration, note.conteudo, note.id, onSave, originalExpiration]);
  ...
  return (
    <Box px={{ base: "4", sm: "5" }} py="4" borderBottomWidth="1px" borderColor="#2a2c29" _hover={{ bg: "#20221f" }}>
      <Flex align="flex-start" gap="2">
        <Textarea value={content} onChange={(event) => { setContent(event.target.value); setStatus("pending"); }} aria-label="Conteúdo da nota" placeholder="Escreva sua nota..." autoresize minH="72px" resize="none" disabled={disabled} bg="#141513" borderColor="#3a3d38" />
        <IconButton aria-label="Remover nota" variant="ghost" size="xs" color="#888d85" disabled={disabled} onClick={() => { void onRemove(note.id).catch(() => undefined); }}><Icon as={LuTrash2} /></IconButton>
      </Flex>
      <Flex mt="2.5" direction={{ base: "column", sm: "row" }} align={{ base: "stretch", sm: "center" }} justify="space-between" gap="2">
        <Box>
          ...
          <Input id={`expiracao-${note.id}`} type="datetime-local" value={expiration} onChange={(event) => { setExpiration(event.target.value); setStatus("pending"); }} disabled={disabled} size="xs" maxW={{ sm: "210px" }} bg="#141513" borderColor="#3a3d38" />
        </Box>
        <Text aria-live="polite" fontSize="xs" color={status === "error" ? "#e39b93" : "#7f847c"}>{statusText}</Text>
      </Flex>
    </Box>
  );
}
